...
# Description: This file is used as it is from code given in the lectures. The name
#             is changed.
# References : https://teaching.mrsharky.com/sdsu_fall_2020_lecture07.html#/9/0/6


# Fill the array with 0's or values from data
def fill_array(data):
...
4.) https://www.educba.com/numpy-nan/
...
def categorical_categorical_correlation_ratio(
    x, y, bias_correction=True, tschuprow=False
):
    corr_coeff = numpy.nan
    try:
        x, y = fill_array(x), fill_array(y)
...
def continious_correlation_ratio(categories, values):
