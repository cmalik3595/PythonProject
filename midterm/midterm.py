...
def continuous_or_categorical_predictor(predictor_data):
    ...
    if predictor_string_check or predictor_unique_ratio < 0.05:
        return "Categorical"
    else:
        return "Continuous"
...
def load_file(file_path, response):
    ...
    if ".xlsx" in file_path:
        data_frames = pd.read_excel(file_path)
    else:
    ...
    while response not in column_names and len(response) != 1:
        print("Error: Invalid Response name!!!")
        response = input("\nEnter response name:\n")
    else:
        pass
    ...
    predicts = []
    remove = column_names.index(response)
    predicts = np.delete(column_names, remove)

    return data_frames, response, predicts
...
def results_table(
    results_response_x_predictor, results_brute_force, results_predictor_correlation
):
...
def main(in_file_name, in_response_type, in_bins):
    ...
    data_frames, response, predicts = load_file(in_file_name, in_response_type)
    (
        response_columns,
        response_type,
        response_mean,
    ...
    results_brute_force, results_predictor_correlation = process_predictors_two_way(
        response, predicts_columns, num_bins, response_columns, response_mean
    )
    correlation_matrix(results_predictor_correlation, predicts_columns)
    results_table(
        results_response_x_predictor, results_brute_force, results_predictor_correlation
    )
    return
...
# Comment from Assignment 4: Use command line arguements
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("\nUsage:")
        print("python3 midterm.py <file_path> <response_name> <number of bins>")
        print("Example: python3 midterm.py titanic.csv alone 10")
        sys.exit()
