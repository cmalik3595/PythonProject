...
def response_processing(df, response):
    ...
    if resp_string_check or resp_unique_ratio < 3:
        ...
        resp_plot = px.histogram(response_col)
        resp_plot.write_html(
            file="./brute_force_graphs/response.html", include_plotlyjs="cdn"
        )
...
def predictor_processing(
    ...
    for pred_name, pred_data in predicts_col.iteritems():
        ...
        relate_file_save = (
            f"./brute_force_graphs/{response_html}_{pred_name_html}_relate.html"
        )
        relate_file_open = (
            f"../../brute_force_graphs/{response_html}_{pred_name_html}_relate.html"
        )
        ...
        reg_fig.write_html(
            file=f"./brute_force_graphs/{pred_name}_regression.html",
        ...
        reg_file_save = (
            f"./brute_force_graphs/{response_html}_{pred_name_html}_reg.html"
        )
        reg_file_open = (
            f"../../brute_force_graphs/{response_html}_{pred_name_html}_reg.html"
        )
        ...
        fig_diff_file_save = (
            f"./brute_force_graphs/{response_html}_{pred_name_html}_diff.html"
        )
        fig_diff_file_open = (
            f"../../brute_force_graphs/{response_html}_{pred_name_html}_diff.html"
        )
...
def pred_processing_two_way(response, predicts_col, bin_n, response_col, resp_mean):
    ...
    for comb in combs:
        ...
        fig_dmr_file_save = (
            f"./brute_force_graphs/{pred_name_1_html}_{pred_name_2_html}_dmr.html"
        )
        fig_dmr_file_open = (
            f"../../brute_force_graphs/{pred_name_1_html}_{pred_name_2_html}_dmr.html"
        )
        ...
        relate_file_open_1 = (
            f"../../brute_force_graphs/{response_html}_{pred_name_1_html}_relate.html"
        )
        relate_link_1 = (
        ...
        relate_file_open_2 = (
            f"../../brute_force_graphs/{response_html}_{pred_name_2_html}_relate.html"
        )
        relate_link_2 = (
...
def corr_matrix(results_pred_corr, predicts_col):
    ...
    for t_comb in type_combs:
        ...
        if var_type_1 == var_type_2 == "Continuous":
            ...
            cat_cont_matrix_save = "./brute_force_graphs/cat_cont_matrix.html"
...
def additional_analysis(
    ...
    hack_flake = int(x.shape[0] * 0.7)
    X_train = x[:hack_flake]
    X_test = x[hack_flake:]
    hack_flake = int(x_pca.shape[0] * 0.7)
    X_train_pca = x_pca[:hack_flake]
    hack_flake = int(y.shape[0] * 0.7)
    y_train = y[:hack_flake]
    y_test = y[hack_flake:]
